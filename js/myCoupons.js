...
//	alert(canNotUseCouponList);
	...
	for(var i=0;i<canUseCouponList.length;i++){
		// 处理时间
		console.log()
		var interval = parseInt(canUseCouponList[i].end - now);
		var surplusTime;
		if(interval<86400){
			if(interval<3600){
				surplusTime = '1小时';
	...
	for(var i=0;i<canNotUseCouponList.length;i++){
		// 处理时间
		var interval = parseInt(canNotUseCouponList[i].begin - now);
		var surplusTime;
		if(interval<3600){
			surplusTime = '1小时';
		}else{
	...
	$('.box-coupons').on('tap','.box-coupons-can-use',function(){
		...
		window.location.href = "http://new.29mins.com/weixin/page/couponsDetailed.php?dataNums="+dataNums;
	...
	$('.box-coupons').on('tap','.box-coupons-cannot-use',function(){
		...
		var dataNums = $(this).attr('data-nums');
		var notUse = 'notUse'
		window.location.href = "http://new.29mins.com/weixin/page/couponsDetailed.php?dataNums="+dataNums+'&notUse='+notUse;
